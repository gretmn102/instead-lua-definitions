...
---@field dsc (string | fun(obj: Obj): string?) Описание объекта. Оно будет выведено в динамической части сцены, при наличии объекта в сцене. Фигурными скобками отображается фрагмент текста, который будет являться ссылкой в окне INSTEAD. Если объектов в сцене много, то все описания выводятся одно за другим, через пробел.
---@field tak (string | fun(obj: Obj): boolean?) Описание действия, когда игрок кликает на этот объект, когда он лежит в комна
---@field act (string | fun(obj: Obj)) Обработчик события, который вызывается при действии пользователя (действие на объект сцены, обычно – клик мышкой по ссылке). Его основная задача – вывод (возвращение) строки текста, которая станет частью событий сцены, и изменение состояния игрового мира.
...
---@field use (fun(this: Obj, another: Obj): boolean?) Событие, которое вызывается, когда игрок пытается использовать предмет на другой предмет.
...
---@field dsc (string | fun(this: Room)) Описание комнаты, которое выводится один раз при входе или при явном осмотре комнаты. В нем нет описаний объектов, присутствующих в комнате.
...
---@field obj ObjId[] Изначальный список объектов.
